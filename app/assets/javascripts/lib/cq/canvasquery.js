...
(function(window, undefined) {
  ...
  $.Wrapper.prototype = {
    ...
    trim: function(color) {
      ...
      } else this.crop(bound[0], bound[1], bound[2] - bound[0], bound[3] - bound[1]);
    ...
    demosceneText: function(text, x, y, maxWidth, gradient) {
      ...
      for(var i = 0; i < lines.length; i++) {
        var oy = y + i * h * 0.6 | 0;
        var lingrad = this.context.createLinearGradient(0, oy - h * 0.6, 0, oy + h * 0.6 | 0);
    ...
    replaceHue: function(src, dst) {
      ...
      for(var i = 0, len = pixels.length; i < len; i += 4) {
        ...
        if(Math.abs(hsl[0] - src) < 0.05) h = $.wrapValue(hue, 0, 1);
        else h = hsl[0];
